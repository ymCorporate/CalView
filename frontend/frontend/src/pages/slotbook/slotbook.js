...
function CalendarPage() {
    ...
    const handleSlotClick = (startTime, endTime, index) => {
        ...
        setClickedSlots((prev) => ({ ...prev, [index]: !prev[index] }));

        if (clickedSlots[index]) {
                        ...
                        <div className="slot-booking-page">
                            ...
                            {availableSlots.length > 0 ? (
                                availableSlots.flatMap(generateTimeSlots).map((slot, index) => (
                                    <div
                                        key={index}
                                        className={`slot-box ${clickedSlots[index] ? 'clicked' : ''}`}
                                        onClick={() => handleSlotClick(slot.start_time, slot.end_time, index)}
                                    >
